    ...
    def __init__(self, file, *args, **kwargs):
        ...
        if os.path.exists(file):
            with open(file, "r") as f:
                data = json.load(f)
            # Ensure dictionary keys are strings to prevent duplicate integer IDs
...
# Use AutoSaveDict for balances so they persist automatically
balances = AutoSaveDict(DATA_FILE, {})   # Ensures auto-saving and prevents duplicates
...
class BlackjackGame:
    ...
    def add_player(self, user_id: int, bet: int):
        # Ensure a user with this ID isn't already in the game
    ...
    def distribute_pot(self):
        # This method will be called after all players and dealer are done.
        lines = []
...
async def blackjack_start(ctx: Interaction, bet: int):
    ...
    # Convert user_id to string for balances
    user_id = str(ctx.user.id)

    # Ensure user is in balances
    if user_id not in balances:
        balances[user_id] = 1000  # Start them with 1000 chips if they don't exist

    # Auto-refill if 0 chips
    if balances[user_id] <= 0:
        balances[user_id] = 100
        await ctx.response.send_message("You're so bad at blackjack that you hit 0 chips! Here's 100 chips to try again.", ephemeral=True)
    ...
    # Add player to the game (store user_id as int in the game logic)
...
async def blackjack_join(ctx: Interaction, bet: int):
    ...
    # BLOCK joining after cards have been dealt
    if game.dealt_cards:
...
##############################
@bot.slash_command(description="Deal cards. The dealer's first card is revealed.")
async def blackjack_deal(ctx: Interaction):
    ...
    # If the command user is a player, show their hand ephemerally
    ...
    if player:
        ...
        await ctx.response.send_message(
            f"**Dealing cards...**\n"
            f"Your initial hand: {player.hand} (Value: {val})\n"
            f"Dealer shows: {dealer_card} + [Hidden]\n"
            "Use `/blackjack_hit` or `/blackjack_stand` to play!",
            ephemeral=True
        )
        # Public message to indicate cards have been dealt
        await ctx.followup.send(
            "**Cards have been dealt! No more joining allowed.**",
            ephemeral=False
        )
    else:
        dealer_card = f"{game.dealer_hand[0][0]}{game.dealer_hand[0][1]}"
        await ctx.response.send_message(
            f"**Dealing cards...**\nDealer shows: {dealer_card} + [Hidden]\n"
            "Use `/blackjack_hit` or `/blackjack_stand` to play!",
            ephemeral=False
        )
...
async def blackjack_myhand(ctx: Interaction):
    ...
    val = game.hand_value(player.hand)
    await ctx.response.send_message(
        f"Your current hand: {player.hand} (Value: {val})",
        ephemeral=True
    )
...
async def help(ctx: Interaction):
    ...
    embed.add_field(name="/blackjack_deal", value="Deal cards. The player who runs this command will see their hand automatically, others must use /blackjack_myhand.", inline=False)
    embed.add_field(name="/blackjack_myhand", value="Check your hand privately.", inline=False)
    ...
    embed.add_field(name="/blackjack_end", value="Manually ends the current game.", inline=False)
...
async def blackjack_leaderboard(ctx: Interaction):
    ...
    # Ensure unique user entries
    unique_balances = {int(uid): bal for uid, bal in balances.items()}

    sorted_bal = sorted(unique_balances.items(), key=lambda x: x[1], reverse=True)
